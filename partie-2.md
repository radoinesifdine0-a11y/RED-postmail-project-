...
**Prérequis:** Avoir complété la Part 1 ([storeparcelapi.md](http://storeparcelapi.md)) avec P95 <60ms validé
...
**HTTP Method:** `PUT` **Path:** `/store_parcels/parcel/{parcelId}` **Content-Type:** `application/json`
...
**HTTP Method:** `DELETE` **Path:** `/store_parcels/parcel/{parcelId}`
...
stateDiagram-v2
    ...
    PENDING --> IN_TRANSIT : Expédition
    PENDING --> CANCELLED : Annulation

    IN_TRANSIT --> READY_FOR_PICKUP : Arrivée magasin
    IN_TRANSIT --> CANCELLED : Annulation

    READY_FOR_PICKUP --> PICKED_UP : Client récupère
    READY_FOR_PICKUP --> EXPIRED : Délai dépassé
    READY_FOR_PICKUP --> CANCELLED : Annulation

    PICKED_UP --> DELIVERED : Livraison confirmée
    PICKED_UP --> RETURNED : Retour client

    EXPIRED --> RETURNED : Retour entrepôt
    EXPIRED --> CANCELLED : Annulation
